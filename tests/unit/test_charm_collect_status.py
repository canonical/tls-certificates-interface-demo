...
class TestCharmCollectStatus:
    ...
    def test_given_tls_integration_not_created_when_collect_unit_status_then_status_is_blocked(
        self,
    ):
        ...
        assert state_out.unit_status == BlockedStatus("certificates integration not created")
    ...
    def test_given_cant_connect_to_container_when_collect_unit_status_then_status_is_waiting(self):
        ...
        container = scenario.Container(name="nginx", can_connect=False)
        state_in = scenario.State(leader=True, containers=[container], relations=[tls_integration])
